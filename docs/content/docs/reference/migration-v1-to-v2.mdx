...
The companion `filterByHasTestId(id)` matches rows that *contain* a descendant with `data-testid={id}`. See [Lists](/guides/lists/) for the full distinction.
...
Use the *initializer form* for `PageObject` subclasses. The factory-argument form is reserved for plain classes whose constructor accepts a `Locator` (see [Custom Controls](/guides/custom-controls/)).
...
- [Lists](/guides/lists/) — `filterByItemTestId` vs `filterByHasTestId`.
- [Custom Controls](/guides/custom-controls/) — factory-argument form.
- [Built-In POM](/guides/built-in-pom/) — initializer form for `PageObject`.
