...
**Adding an accessor to an existing page object** is the same first two calls: check the tree so you do not duplicate one, then `map_coverage {"class": "…", "buckets": ["uncoveredTestIds"]}` for rendered IDs nothing selects yet. Each entry carries a ready-to-paste `suggestion` such as `@Selector("ApplyPromoButton")` and every source site under `occurrences[].loc`; `conditional` and `repeated` on the occurrence help distinguish singleton from list-shaped UI.
...
- **Every occurrence `reach: "component-prop"`.** The ID is written on a component tag and nothing proved the component forwards it to a host element. It may not be in the DOM at all — read the component before writing a selector for it, or re-call with `assumeForwarded: true` if your design system forwards as a rule. [`--assume-forwarded`](/mcp/configuration/#when-should-i-assume-prop-forwarding) sets that default; `false` restores conservative interpretation for one call.
...
| `uncoveredTestIds` without `includeRawLocators: true` | Not "untested" — direct `getByTestId`-family calls and exact test-ID fragments inside page-object `locator()` expressions were not scanned |
