...
| v1 suites read partially, and API hints are v2-only | [all five](#does-this-work-with-a-v1-test-suite) |
...
| `map_coverage` | **Works, with a flag.** Pass `includeRawLocators: true` so plain `getByTestId()` calls count as selectors. In discovered page-object classes, it also extracts exact test-ID equality fragments from `locator()` expressions. Without the flag the raw side is empty and every rendered ID reads as uncovered. |
...
### Does the scan include test files?
...
What stays invisible is a page object in a suite that never uses this library - locators declared in a constructor or as ordinary properties, in a class that extends nothing of ours and appears in no fixture map. If `list_page_objects` comes back empty, the response first names the selected TypeScript program and checks sibling configs for direct library imports:

> 0 page objects in the analysed program (tsconfig.json, N files). e2e/tsconfig.json covers M file(s) that import "playwright-page-object". Restart the MCP server with `--tsconfig "e2e/tsconfig.json"`.

When a candidate exists, the problem is the selected program, not proof that the repository has no page objects. When none exists, the hint says so; then check whether the selected config includes the files and whether they directly import the package. `--src-dir` restricts a program rather than widening it, so use it only after choosing the correct tsconfig and only when you can name every app and test directory that must remain.
...
Prop forwarding is resolved one hop. A test ID written as a prop on a component tag is **not** counted as rendered unless forwarding to a host element was proven, because a component is free to ignore a prop. Per-call [`assumeForwarded`](/mcp/configuration/#when-should-i-assume-prop-forwarding) turns that into an explicit assumption when you know your codebase forwards; `--assume-forwarded` supplies the server default.
