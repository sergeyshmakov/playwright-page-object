...
Fragments work because the [context resolution](/reference/context-resolution/) priority looks at the host's `locator` property before falling back to `page`. The parent's `@Selector(..., FragmentClass)` constructs `new FragmentClass(resolvedLocator)` and the resolved locator becomes the fragment's scope.
...
- [Custom Controls](/guides/custom-controls/) — fragments and custom controls share the same constructor shape.
- [Context Resolution](/reference/context-resolution/) — the resolution rules that make fragments work.
