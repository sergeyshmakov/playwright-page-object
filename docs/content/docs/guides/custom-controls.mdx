...
This protects the `cloneWithContext` nesting contract that `PageObject` relies on. See [Built-In POM](/guides/built-in-pom/) for the initializer form.
...
- [Built-In POM](/guides/built-in-pom/) — for `PageObject`-based controls with wait helpers.
- [Fragments](/guides/fragments/) — the same constructor shape, used for sub-trees with their own decorators.
