...
Microsoft's [Playwright MCP](https://playwright.dev/mcp/introduction) drives a live browser through accessibility snapshots. This server never opens a browser. It analyzes page-object classes and JSX or TSX source so an agent can reuse existing test APIs and selectors while writing code. The two servers solve different parts of the same workflow.
...
<BlogFaq>
<BlogFaqItem
  id="does-the-mcp-server-work-with-data-tid-or-other-custom-attributes"
  title="Does the MCP server work with data-tid or other custom attributes?"
>
...
<BlogFaqItem
  id="does-it-execute-my-code-or-launch-a-browser"
  title="Does it execute my code or launch a browser?"
>
...
<BlogFaqItem
  id="do-i-need-the-playwright-page-object-decorators-for-it-to-be-useful"
  title="Do I need the playwright-page-object decorators for it to be useful?"
>
...
<BlogFaqItem
  id="which-mcp-clients-does-it-support"
  title="Which MCP clients does it support?"
>

Any MCP client that can launch a local stdio server, including Claude Code, Cursor, VS Code, and Codex CLI. The server speaks the standard protocol over stdin/stdout. Registration snippets for each client are in the [quick start](/mcp/quick-start/).

</BlogFaqItem>
<BlogFaqItem
  id="does-it-work-in-a-monorepo"
  title="Does it work in a monorepo?"
>
...
<BlogFaqItem
  id="how-fresh-are-the-results"
  title="How fresh are the results?"
>
