...
| A UI section repeats across pages | Make it a [fragment](/guides/fragments/) | Constructor takes `locator: Locator`; nested decorators chain from it |
...
| You want `waitVisible()`, `.expect()`, filter chains | Use [`PageObject`](/api/page-object/) / [`ListPageObject`](/api/list-page-object/) | Built-in wait helpers and assertions |
...
Three similar `.fill()` calls is not duplication; five matching wait blocks across files is. See [Incremental Adoption](/guides/incremental-adoption/) for a longer walkthrough.

## Next

- [Plain Classes](/guides/plain-classes/) — the default pattern in depth.
- [Fragments](/guides/fragments/) — reusable sub-trees.
- [Custom Controls](/guides/custom-controls/) — integrating existing control classes.
- [Built-In POM](/guides/built-in-pom/) — when wait helpers and `.expect()` pay back.
