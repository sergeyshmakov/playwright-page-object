...
The built-in POM layer adds wait helpers, soft assertions, and a typed `.expect()` API on top of the locator-composition primitives. Use it when these helpers save real code; use [plain classes](/guides/plain-classes/) when they do not.
...
For subclasses with custom constructor arguments, implement `cloneWithContext()` — see the [PageObject API reference](/api/page-object/).

## When to pick this style

- You want wait helpers and `.expect()` on accessors without writing them yourself.
- You're working with lists and want `filterByText`, `getItemByTestId`, async iteration — see [Lists](/guides/lists/).
...
- You already have a control library — see [Custom Controls](/guides/custom-controls/).

## See also

- [PageObject API](/api/page-object/) — full method reference.
- [RootPageObject API](/api/root-page-object/) — root-only methods and constructor contract.
- [Lists](/guides/lists/) — `ListPageObject` for repeated controls.
