...
- a `--src-dir` that is not a glob and not an exclusion does not exist, or resolves outside `--project-root` (the workspace ignores outside-root paths, so such a scope would silently contribute nothing).
...
`--src-dir` can only ever narrow. A path resolving outside `--project-root` is refused at startup, so it is not the way to reach a shared UI package in a sibling directory — re-root the server instead, as described under [monorepos](#how-do-i-configure-a-monorepo). When component tags resolve to sources outside the root, the `ui-scope-incomplete` warning names the directory to pass to `--project-root`.
...
The default is **8000**, which covers a large application and the monorepo root above it. It was 2000, and that refused every call on the repositories this server exists for — and because it is a startup flag, an agent that hits it cannot do anything but stop and ask you to edit your MCP config. The failure is asymmetric: too low and the tool does not work, too high and a scan uses memory you can see and that is released after ten idle minutes.

A scan past 3000 files reports itself with a `large-scan` info diagnostic naming the count, since that is most of the server's memory and all of its cold start. Lower `--max-files` if you would rather an oversized scope fail fast than run.
...
One number moves in the direction you may not expect: `summary.uiTestIds` **falls**. An id written as a prop and the host element it reaches are one id once forwarding is assumed, where before they were counted separately — on one production app, 1,571 → 1,287. Nothing was lost; two half-facts merged into one. `matched` rises sharply over the same call (627 → 1,430), and `coverage` barely moves, because the assumption grows the numerator and the denominator together.
