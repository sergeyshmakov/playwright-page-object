...
The cap governs files **parsed**, not files analyzed: every source the project holds, less declaration files and ignored directories such as `node_modules` and build output. Your include and exclude patterns are not applied to it. `--src-dir` says which files are analyzed, and the resolver still parses what analyzed code imports — an alias target or a `.js` module outside the scope is parsed, retained, and counted. Counting only the narrowed scope let a project sitting on the cap pull in unlimited siblings for free.
