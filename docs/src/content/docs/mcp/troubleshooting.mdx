...
An index that is genuinely empty has two usual causes. Either the page objects are outside the scanned scope, which `--src-dir` fixes, or they do not import from `playwright-page-object` at all. Discovery is evidence-based: a class is indexed because it carries a library decorator, extends a library base class, or is bound in a fixture. A class whose *only* evidence is being passed as a factory argument is a control: `map_coverage` analyses it, and `list_page_objects` deliberately leaves it out, so a control can be absent from the index and present in the coverage scan. A class that re-exports decorators through a local wrapper module is not recognised.
...
`meta.fidelityReason` carries exact per-reason counts. The individual per-site entries in `meta.warnings` are a **sample** — a deep page hits the depth limit at dozens of places, and repeating that once per site cost a third of the response — and `tree-partial` says so and gives the true totals.

A partial tree that reached **no** ID at all is not sent: `roots` comes back empty with `meta.suppressed` explaining, because a cut walk that found nothing proves nothing about what renders there. Use `testId` to look an ID up across the whole scan, which is complete whatever the walk did.
...
| was rebuilt | The workspace was released after an idle period, or this call analyses a different scope. **Nothing in your sources changed** — re-mint and carry on |
...
Every code the server can put in `meta.warnings`. They are part of the wire contract, so they are safe to match on. Severity varies by situation for several of them — `playwright-config-ambiguous` is informational when the attribute resolved anyway and a warning when it did not — so read `severity` rather than assuming.
...
| `large-scan` | The scan parses more than 3,000 files — most of the server's memory and all of its cold start. Narrow with `--src-dir`, or lower `--max-files` to make an oversized scope fail fast |
...
| `forwarding-assumed` | `--assume-forwarded` is on; the message counts the IDs it promoted, and every affected entry is labelled |
