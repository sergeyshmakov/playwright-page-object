...
`too_large` means the serialized response exceeded the 40,000-character cap. Use the narrowing options that match the call mode:
...
Switching to `outline` is often enough for a model-readable tree. The outline omits visibility, JSDoc, column numbers, and some structured fields, so keep JSON when another program will parse the response.
