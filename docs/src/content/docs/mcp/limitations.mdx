...
| Limitation | Affects |
| --- | --- |
| Page-object tools only see classes with selector decorators | [`list_page_objects`, `get_page_object_tree`](#can-i-use-the-server-without-selector-decorators) |
...
Yes, partly. You can point the server at any Playwright repository and get real answers on the first call, but two of the five tools have nothing to report until selectors are declared with decorators.
...
| `list_page_objects` | Returns an empty array plus a hint. Nothing is indexed. |
| `get_page_object_tree` | Nothing to address. There is no class for it to return. |

So the honest summary is: **anyone can start, and the test-ID half of the server is useful immediately.** The page-object half is what decorators unlock.
...
The **member decorators**, not the base class. A class is indexed when its accessors use `@Selector`, `@ListSelector`, `@RootSelector`, or a `@SelectorBy*` variant.

It does not have to extend `PageObject` or `RootPageObject`. The example app ships `PlainHostCheckoutPage`, a plain class whose only inherited member is `readonly page`, and `list_page_objects` indexes it as host kind `pageFallback` with all six of its members. See [Plain Classes](/guides/plain-classes/) for that style and [Incremental Adoption](/guides/incremental-adoption/) for moving an existing suite one accessor at a time.

What stays invisible is a page object that declares its locators in a constructor or as ordinary properties, because there is no decorator for the analysis to read. If `list_page_objects` comes back empty on a repository that clearly has page objects, that is the reason, and the response says so:
