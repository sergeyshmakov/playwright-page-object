...
The server tells you. Any response whose walk hit a module outside the scan carries `ui-scope-incomplete`, and it names the modules:
...
- **`--max-files` defaults to 8000.** A large monorepo can exceed it, at which point the scan is capped and results are drawn from an arbitrary subset. Raise it, or narrow with `--src-dir`.
...
Explicit `--tsconfig` wins. Otherwise the server takes the nearest `tsconfig.json` walking up from the Playwright `testDir` toward the project root, which in a monorepo is usually the e2e tsconfig next to the specs rather than the root one. See [Configuration](/mcp/configuration/#how-does-the-server-choose-a-tsconfig) for the full order.
