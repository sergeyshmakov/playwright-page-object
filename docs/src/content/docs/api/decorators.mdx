...
Every decorator returns a Playwright locator built from the host's resolved root. See [Context Resolution](/reference/context-resolution/) for how the root is determined.
...
See [Custom Controls](/guides/custom-controls/) and [Built-In POM](/guides/built-in-pom/) for the two non-raw forms.
...
- [Context Resolution](/reference/context-resolution/) — root resolution priority.
- [PageObject](/api/page-object/) — methods on `PageObject` accessors.
- [ListPageObject](/api/list-page-object/) — methods on `ListPageObject` accessors.
