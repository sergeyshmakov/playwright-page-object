...
`RootPageObject` extends `PageObject`, so all the [`PageObject` members](/api/page-object/) are available:
...
For plain page objects without those needs, use a plain class with `@RootSelector(...)`. See [Plain Classes](/guides/plain-classes/).
...
- [PageObject](/api/page-object/) — the base class.
- [Decorator API](/api/decorators/) — all root decorators.
- [Built-In POM guide](/guides/built-in-pom/) — walkthrough.
