...
const API_LINES: Record<ApiSurface, string> = {
	members:
		'Every member listed here is a property: `po.CartItems`. Read its `result` before calling it. `locator` IS a Playwright Locator - call Playwright on it directly (`po.PromoCodeInput.fill("SAVE20")`). `pageObject` and `list` are library page objects and have the API below, including `.$` for the raw locator (`po.ApplyPromoButton.$.click()`). `control` is a project class, and everything below is conditional for it: `.$` is defined on PageObject like the waits are, so a control that does not extend PageObject has none of it. Its own node in `defs` shows what it extends - read that rather than assuming. `unknown` was not statically resolvable, so nothing here applies to it; the payload carries its source text instead.',
	RootPageObject:
		"If the node lists `fixtures`, that binding already exists - take it as a test argument (`test(..., async ({ checkoutPage }) => ...)`), bound by `createFixtures({ checkoutPage: CheckoutPage })`. Otherwise construct it from the Playwright page, `const po = new CheckoutPage(page)` - but check the node's `ctorSignature` first: a class with extra required parameters needs them, and `new CheckoutPage(page)` would not compile.",
	PageObject:
