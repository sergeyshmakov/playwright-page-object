...
Scope: the page-object tools - list_page_objects and get_page_object_tree - index a class on any one of four kinds of evidence: a selector decorator (@Selector, @ListSelector, @RootSelector, @ListRootSelector, @SelectorBy* / @RootSelectorBy*), extending one of this library's base classes (PageObject, ListPageObject, RootPageObject), a binding in a createFixtures map, or being passed as a decorator factory argument. Any one suffices - a decorated plain class is indexed, and so is an undecorated class that only extends PageObject. One exception: a control whose ONLY evidence is the factory argument is left out of list_page_objects, though get_page_object_tree still expands it inline and the coverage tools still read it. What they cannot see is a suite that never uses this library, so an empty list_page_objects means no such class is declared in scope, not that the repository has no page objects. Report that distinction rather than concluding there are none, and never invent a replacement for a page object you could not see. The other tools need none of this: get_testid_tree reads JSX/TSX and works on any repository, and map_coverage judges direct getByTestId-family calls plus exact test-id attributes inside locator() expressions in discovered page-object classes once you pass includeRawLocators: true - off by default, so without it a suite built on raw locators reads as entirely uncovered.
...
const TESTID_DESCRIPTION = `Returns the tree of test-id attributes rendered by the app's components (JSX/TSX), in nesting order, including ids built from template literals (reported as patterns like CartItem_* with the source expression) and, when followComponents is true, ids contributed by child components imported from other files. The attribute name comes from the Playwright config (use.testIdAttribute), defaulting to data-testid. Use it before writing any selector so you reference ids that actually exist and nest them correctly, when a locator resolves to zero elements, and when deciding between @Selector and @ListSelector for a repeated row. Address by "file", by "component" name, or by both when two files declare the same component name; or pass "testId" to find where a known id is rendered. Ids rendered inside a conditional are flagged conditional; ids inside .map() are flagged repeated. One-hop prop forwarding is resolved; anything deeper is marked unresolved rather than guessed. On a testId lookup, assumeForwarded overrides the server default for interpreting prop-only occurrences as rendered; even when true, their source evidence remains reach: "component-prop", and false restores the conservative interpretation. Returns format:"${FORMAT_DEFAULT}" unless you ask otherwise: outline is indented text costing a fraction of the tokens, json carries every node and field for a program to parse.`;

const COVERAGE_DESCRIPTION = `Cross-references the selectors declared by page-object classes against the test ids actually rendered in the app source and returns: matched (with match confidence), uncoveredTestIds (rendered ids no page object references - gaps you could fill, each with every render occurrence and a ready-to-paste decorator suggestion), deadSelectors (selectors whose test id is rendered nowhere - broken or stale locators, with nearest-id typo suggestions), nonTestIdSelectors (role/text/label selectors - reported for awareness, NEVER counted as dead since they cannot be checked statically), and unknown buckets for dynamic values and for ids passed to components that never provably forward them to the DOM. Machine-readable bucket keys: matched entries use selector, ui and confidence; uncoveredTestIds use id (not testId), patternSource, occurrences, suggestion and optional assumed, and every occurrences[] item carries loc.file, loc.line, optional loc.column, plus conditional/repeated when true; deadSelectors use defId, memberPath, loc, text, origin and nearestTestIds; nonTestIdSelectors use kind, defId, memberPath, loc and text; unknownSelectors use defId, memberPath, loc, reason, raw, origin and evidence; unknownTestIds use reason, occurrence and patternSource. Use it after refactoring a component or renaming test ids, before adding accessors to an existing page object, and when a test times out on a locator - it tells you whether the id disappeared from the UI. Set includeRawLocators: true to count direct getByTestId-family calls and exact [<attribute>="id"] fragments inside locator() expressions in discovered page-object classes; unresolved dynamic fragments are not guessed. Omit class and file to scan the whole project. assumeForwarded overrides the server default for this report, including false to restore conservative classification; every affected ID carries assumed: true and every affected match carries forwarding: "assumed". summary.coverage is null rather than 1 when nothing was matchable, and scope reports what the two sides were drawn from - read the warnings before acting on a number. Request only the lists you need with buckets; buckets: [] returns summary and scope alone, which is the cheapest way to see the shape of the answer before asking for a list. Every response carries meta.coverageId, an opaque handle to this exact report that query_coverage pages one bucket at a time without restating class / file / attribute / forwarding interpretation. ${HANDLE_LIFETIME_TEXT} A page too large for the response cap is trimmed rather than refused: meta.truncatedBuckets names what was cut and meta.nextOffset says where to resume.`;

const QUERY_DESCRIPTION = `Pages one bucket of a coverage report that map_coverage already built, addressed by the opaque meta.coverageId that call returned. Takes coverageId, one bucket name, offset and limit, and returns that bucket's page alongside the same summary map_coverage ships, so a capped page still reports every bucket's real size. scope ships on the first page only (offset 0): the handle pins one snapshot, so it is identical on every later page and map_coverage already gave it to you. Use it to walk a long list to its end - copy meta.nextOffset into the next call's offset and stop when that key stops coming back - and to avoid restating the class / file / attribute / includeRawLocators / assumeForwarded scope of the original call, which the handle carries. It also holds the report still: the same id answers from the same snapshot every time, and if the sources change underneath it the call fails with expired_handle instead of quietly renumbering the list your offsets point into. ${HANDLE_LIFETIME_CLAUSE}`;
...
export function createMcpServer(options: McpServerOptions): McpServer {
	...
	server.registerTool(
		...
		safeHandler((args) =>
			handleGetTestIdTree(getWorkspace(), args, {
