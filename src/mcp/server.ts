...
Scope: the page-object tools - list_page_objects and get_page_object_tree - see only classes whose accessors carry this library's selector decorators (@Selector, @ListSelector, @RootSelector, @ListRootSelector, and the @SelectorBy* / @RootSelectorBy* variants). Extending PageObject is not required; a plain class with decorated accessors is indexed. An empty list_page_objects therefore means none are declared in scope - not that the repository has no page objects. Report that distinction rather than concluding there are none, and never invent a replacement for a page object you could not see. The other tools need no decorators at all: get_testid_tree reads JSX/TSX and works on any repository, and map_coverage judges plain getByTestId() calls once you pass includeRawLocators: true - off by default, so without it a suite built on raw locators reads as entirely uncovered.
